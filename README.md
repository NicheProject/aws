...
volume as a filesystem or logical volume.
    ...
    niche_aws_ebs_volume "db_ebs_volume" do
...
This will create a 50G volume, attach it to the instance as `/dev/sdi`.
    ...
    niche_aws_ebs_volume "db_ebs_volume_from_snapshot" do
      aws_access_key aws['aws_access_key_id']
      aws_secret_access_key aws['aws_secret_access_key']
      size 50
      device "/dev/sdi"
...
Then to set up the Elastic IP on a system:
    ...
    niche_aws_elastic_ip "eip_load_balancer_production" do
      aws_access_key aws['aws_access_key_id']
      aws_secret_access_key aws['aws_secret_access_key']
...
For example, to register the node in the 'QA' ELB:
    ...
    niche_aws_elastic_lb "elb_qa" do
      aws_access_key aws['aws_access_key_id']
      aws_secret_access_key aws['aws_secret_access_key']
      name "QA"
      action :register
...
Assigning tags to a node to reflect it's role and environment:
    ...
    niche_aws_resource_tag node['ec2']['instance_id'] do
      aws_access_key aws['aws_access_key_id']
      aws_secret_access_key aws['aws_secret_access_key']
...
disk set:
    ...
    niche_aws_resource_tag 'my awesome raid set' do
      aws_access_key aws['aws_access_key_id']
      aws_secret_access_key aws['aws_secret_access_key']
...
**Note** If you would like to use the normal attribute
   ...
   niche_aws_ebs_volume resource examples above as input for the resource_id attribute of
   the niche_aws_resource_tag resource, you must employ the "lazy" attribute feature
...
``` ruby
niche_aws_resource_tag "db_ebs_volume" do
